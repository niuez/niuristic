...
例: 指数分布からN個とってソートした配列の分布

ソートされた状態から一つずつ要素を変化させるGibbs Samplingで分布からサンプルすることができる。

- `simple_sampling`は12.85sec
- `fast_gibbs_sampling`は3.165sec
    - 逆関数法を用いて指数分布の限定領域からサンプル
    - サンプルした値ではなく累積確率密度を持つことで高速化
