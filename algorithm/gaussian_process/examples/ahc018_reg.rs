...
fn main() {
    ...
    let window = 20;
    let mut gp_builder = GaussianProcessBuilder::new(0.1, 0.01, 0.001);
    for i in 0..(n / window) {
        for j in 0..(n / window) {
            gp_builder.add_data(
                dvector![(i * window) as f64 / n as f64, (j * window) as f64 / n as f64],
                (field[i * window][j * window] as f64).sqrt()
            );
        }
    }

    let reg = gp_builder.build();
    for i in 0..n {
        for j in 0..n {
            let (m, v) = reg.regression(&dvector![(i as f64) / (n as f64), (j as f64) / (n as f64)]);
            let x = (m * m).clamp(10.0, 5000.0) as usize;
            print!("{} ", x);
        }
        println!();
    }
